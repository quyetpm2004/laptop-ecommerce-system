...
const handleAddProductToCart = async (quantity: number, productId: number, user: any) => {
    // has card
...
const handleDeleteCartDetail = async (id: number, sumCart: number, userId: number) => {
...
const updateCartDetailBeforeCheckout = async (data: {id: string; quantity: string}[], userId) => {
    var newQuantity = 0
...
const handlePlaceOrder = async (userId: number, receiverName: string, receiverAddress: string, receiverPhone: string, totalPrice: number) => {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if(cart) {
            ...
            const dataDetails = cart?.cartDetails?.map(
                item => ({
                    price: item.price,
                    quantity: item.quantity,
...
export {handleGetAllProducts, handleGetDetailProduct, handleAddProductToCart, 
    handleGetCartByUserID, handleGetCartDetail, handleGetProductInCart, 
    handleDeleteCartDetail, updateCartDetailBeforeCheckout, handlePlaceOrder,
